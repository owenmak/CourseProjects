...
int main(int argc, char** argv) {
	...
	if(type[tbl_column_count(ttbl) - 1] == 'S') {
		if(ca_defined("class"))
			charClass = ca_str_value("class");
		tbl = string_mapping(ttbl, vtbl, !ca_defined("class"));
	}
	else {
		if(ca_defined("class")) {
			intClass = ca_int_value("class");
		}
		tbl = int_mapping(ttbl, vtbl, !ca_defined("class"));
	}
//	printf("%d\n",uniqueClass);
//	tbl_print(tbl[0]);
//	printf("\n\n\n");
...
//	tbl_print(tbl[0]);
	...
	if(!ca_defined("class"))
		...
		int error = 0;
//		printf("finalClass:%d, uniqueClass:%d\n",finalClass, uniqueClass);
		for(int i = 1; i <= uniqueClass; i++) {
			if(i == finalClass + 1)
				continue;
			error += cm->matrix[finalClass+1][i];
		}
		printf("%d\n",error);
	}
	tbl_free(ttbl);
