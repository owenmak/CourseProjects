...
double find_double_split_value(Column *column) {
	...
	for(int i = min2; i < column->count; i++) {
		if(validCol[i] == 0)
			continue;
		if(entropy[min2] > entropy[i] && entropy[i] > entropy[min])
			min2 = i;
	}
	free(entropy);
	free(validCol);
	return (column->value->db[min] + column->value->db[min2]) / 2;
}
