...
public class CarLot{
    ...
    public boolean[] getCars(String make){
        boolean[] getCars = new boolean[cars.size()]; 
        for (int i = 0; i < cars.size(); i++) {
          if (cars.get(i).getMake().equals(make) && !cars.get(i).isSold())
            getCars[i] = true;
          else
            getCars[i] = false;
        }
    ...
    /**
     * add a new car to the array and increment the car counter
     *
     * @param String color
